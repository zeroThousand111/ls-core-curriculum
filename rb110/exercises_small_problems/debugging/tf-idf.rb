...
def tfidf(term, document, documents)
  # binding.pry
  tf(term, document) * idf(term, documents) # issue is with idf return value
end
...
def idf(term, documents)
  ...
  Math.log(number_of_documents / number_of_documents_with_term.to_f) # the problem is here
end
...
# expected outputs:
puts tfidf("cat", document1, documents) # ~ 1.2 | cat x 3 in doc1 | cat x 7 in documents | tfidf 3 * 0.0 = 0.0 | DOES NOT PERFORM AS EXPECTED
puts tfidf("cat", document2, documents) # ~ 1.6 | cat x 4 in doc2 | cat x 7 in documents | tfidf 4 * 0.0 = 0.0 | DOES NOT PERFORM AS EXPECTED
puts tfidf("cat", document3, documents) # 0 | cat x 0 | PERFORMS AS EXPECTED

puts tfidf("quantum", document1, documents) # ~ 5.5 | quantum x 5 in doc1 | x 5 in documents | PERFORMS AS EXPECTED
puts tfidf("quantum", document2, documents) # 0 | PERFORMS AS EXPECTED
puts tfidf("quantum", document3, documents) # 0 | PERFORMS AS EXPECTED

puts tfidf("mastery", document1, documents) # 0 | PERFORMS AS EXPECTED
puts tfidf("mastery", document2, documents) # 0 | PERFORMS AS EXPECTED
puts tfidf("mastery", document3, documents) # ~ 4.4 | PERFORMS AS EXPECTED

puts tfidf("some", document1, documents) # 0 | PERFORMS AS EXPECTED
puts tfidf("some", document2, documents) # ~ 0.4 | DOES NOT PERFORM AS EXPECTED
puts tfidf("some", document3, documents) # ~ 0.4 | DOES NOT PERFORM AS EXPECTED