...
class Length
  ...
  def <=>(other)
    if self > other # invoking the `Length#>` custom method above
      return 1
    elsif self == other # `Length#==`
      return 0
    elsif self <(other) # `Length#<`
      return -1
    end
...
=begin
Solution:
Array#sort uses the results from the `<=>` operator of whatever objects are being compared.  Our custom `Length` objects don't have a `Length#<=>` instance method yet, so I wrote one, using the custom methods already written `Length#>`, `Length#==` and `Length#>`
=end
